...
public class SyncNetworkUtils {
     ...
     * 
     * public static void addExtraMarkedField(Class clazz, Field field){ List<Field> markedFields = extraMarkedFields.get(clazz); if(markedFields ==
     * null) { markedFields = new ArrayList<Field>(); extraMarkedFields.put(clazz, markedFields); } markedFields.add(field); }
     */
    ...
    private static List<SyncedField> getSyncedFieldsForField(Field field, Object te, Class searchedAnnotation) {
        ...
        if (syncedField != null) {
            ...
            Object o;
            try {
                int filteredIndex = field.getAnnotation(FilteredSynced.class) != null ? field.getAnnotation(FilteredSynced.class).index()
                        : -1;
