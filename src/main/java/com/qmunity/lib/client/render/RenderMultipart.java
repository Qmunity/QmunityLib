...
public class RenderMultipart extends TileEntitySpecialRenderer implements ISimpleBlockRenderingHandler{
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer){
        ...
        if(te != null) for(IPart p : te.getParts())
            if(p.getParent() != null && p instanceof IPartRenderable) if(((IPartRenderable)p).shouldRenderOnPass(PASS)) ((IPartRenderable)p).renderStatic(new Vec3i(x, y, z), renderer, PASS);
